...
void ResultScene::Initialize() {
    ...
    if (auto audio = Game::GetInstance()->GetAudio()) {
        audio->Play("BGM_RESULT", true, 0.4f);
    }
    if (isClear) {
...
void ResultScene::Draw() {
    ...
    uint32_t gridCol = (static_cast<uint32_t>(gridAlpha * 255.0f) << 24) | (isClear ? 0x0000FFFF : 0x00FF0000);
    ...
    float scale = 1.0f + sinf(rs_blinkTimer * 3.0f) * 0.05f;
    ...
    if (isClear) items = { L"RETRY", L"STAGE SELECT", L"TITLE" };
    ...
    for (size_t i = 0; i < items.size(); ++i) {
        ...
        if (isSelected) {
            x += 20.0f + sinf(rs_blinkTimer * 5.0f) * 5.0f;
