...
namespace EntityFactory {
    ...
    inline void CreateShape(EntityID id, World* world, ShapeType type) {
        // GeometryGenerator側で定義された生成ロジックを呼び出す
        MeshData data = GeometryGenerator::CreateMesh(type);
    ...
    inline EntityID CreatePlayer(World* world, 
        ...
        float sx = 1.0f, float sy = 1.0f, float sz = 1.0f) {
            ...
            // 基準サイズ (GeometryGeneratorで作るサイズと同じにする)
            ...
            // メッシュの設定 (カプセル)
            CreateShape(entity, world, ShapeType::CAPSULE);
        return entity;
    }
    ...
    inline EntityID CreateEnemy(World* world, 
        ...
        // メッシュの設定 (キューブ)
        CreateShape(entity, world, ShapeType::CUBE);
